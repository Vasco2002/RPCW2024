...
alunos = Namespace("http://www.semanticweb.org/vasco/ontologies/2024/3/alunos/")
...
for aluno in data["alunos"]:
    ...
    g.add((aluno_uri, alunos.nota_projeto, Literal(aluno["projeto"], datatype=XSD.int)))

    # Adicionar notas de TPC
    for tpc in aluno["tpc"]:
        tpc_uri = format_uri(aluno["idAluno"] + "_" + tpc["tp"])
        g.add((tpc_uri, RDF.type, alunos.TPC))
        g.add((tpc_uri, alunos.nota_tpc, Literal(tpc["nota"], datatype=XSD.float)))
    ...
    for tipo, nota in aluno["exames"].items():
        ...
        g.add((exame_uri, alunos.nota_exame, Literal(nota, datatype=XSD.int)))
